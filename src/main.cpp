...
#if defined(NDEBUG)
# error "Lissomcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x57f78538a4bd3e8a538d6a4b3ce2470eae31b109ce4b3dd6ffabe233814f4dd2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lissomcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Lissomcoin Signed Message:\n";
...
{
    // Lissomcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Lissomcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Lissomcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Lissomcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Lissomcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lissomcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd1bd1478ce880884ce07d2fe8e41f33ca4da965f31f1c867d7e0596592486922");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "I Love KH Forever, and I will never forget her";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042f8256216aa54493d9ad955c86fb4942dbaa0cc54a832d4d4f3a544901e9628d41c90e380fef332ec18fc1634bdc41903176c01275bc7f96c51d6e370b20707d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1613321419;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085534361;

        if (fTestNet)
        {
            block.nTime    = 1613321333;
            block.nNonce   = 387351803;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe913c83f017d3dc7b6e1da14ee708780d5d0e0e20dc8e57dda7c6905579c9a2b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf8, 0xc7, 0xb2, 0xd5 }; // Lissomcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LissomcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LissomcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LissomcoinMiner(CWallet *pwallet)
{
    printf("LissomcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lissomcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LissomcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LissomcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LissomcoinMiner, pwallet));
}
